  ...
  return (
    <Link href={`/posts/${post.slug}`}
      ...
      <div className="flex items-center gap-2 mb-3">
        <span className="text-xs font-semibold text-[#16a34a] bg-green-50 px-2 py-0.5 rounded-full border border-green-100">
          {typeLabel[post.type]}
        </span>
        <span className="text-xs text-gray-400">{post.readTime} min</span>
      </div>
      <h3 className="font-bold text-[#111] text-base leading-snug mb-2 group-hover:text-[#16a34a] transition-colors">
        {post.title}
      </h3>
